...
def _build_html_report(session_data: dict, report_type: str,
    ...
    for d in devices:
        ...
        <tr>
            ...
            <td>{d.get('vendor', '-')}</td>
            <td>{d.get('device_type', '-')}</td>
            <td>{d.get('purdue_level', '-')}</td>
            <td>{', '.join(d.get('protocols', []))}</td>
        </tr>"""
...
<body>
    <div class="cover">
        ...
        <div class="meta">
            ...
            <p><strong>Client:</strong> {client_name or 'N/A'}</p>
            <p><strong>Assessor:</strong> {assessor_name or 'N/A'}</p>
            <p><strong>Date:</strong> {now}</p>
            <p><strong>Session:</strong> {session_data.get('name', 'N/A')}</p>
        </div>
    ...
    <div class="footer">
        Generated by Gridwolf v0.9.2 — Passive ICS/SCADA Network Discovery &amp; Topology Visualization<br>
