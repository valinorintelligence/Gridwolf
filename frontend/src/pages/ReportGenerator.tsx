...
export default function ReportGenerator() {
  ...
  const handleDownload = (name: string) => {
    // Generate a text-based report as a downloadable file
    const selectedSections = SECTIONS.filter((s) => sections[s.id]);
    const content = [
      '=' .repeat(60),
      'GRIDWOLF - ICS/OT Security Assessment Report',
      '=' .repeat(60),
      '',
      `Report: ${name}`,
      ...
      'TABLE OF CONTENTS',
      '-'.repeat(60),
      ...selectedSections.map((s, i) => `  ${i + 1}. ${s.label}`),
      '',
      '-'.repeat(60),
      '',
      'This is a demo report. In production, this would contain',
      'full assessment data from passive network analysis including',
      'device inventory, protocol analysis, CVE matches, and',
      'compliance mapping results.',
      '',
      `Generated by Gridwolf v0.1.0 on ${new Date().toISOString()}`,
    ].join('\n');
